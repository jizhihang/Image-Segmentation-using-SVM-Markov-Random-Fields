%thesis
%{{
run('C:\Users\ndy102\Desktop\vlfeat\vlfeat-0\vlfeat-0.9.20/toolbox/vl_setup');
close all;
clc;
clear all;
warning('off','all');
...
%             for cs=5:5:20
    ...
    while(i< length(srcFiles))
        ...
        [data_matrix_training,class_matrix ] = feature_selector_hog(selected_pixels,bbc,I1,nf,nb,cs,bs,ob,fs,choice );
    ...
    [SVMStruct,kkt,rbf_sigma ] = classifier_training(data_matrix,class_matrix_data,rbf,0);
    svm{tp1,1} = SVMStruct;
    %             [Itemp, matf ] = classifier(SVMStruct,ws);
    matf = classifier(SVMStruct,ws,cs,bs,ob,fs,choice)
    matfin{tp1} = matf;
    svm{tp1,2} = matf;
