...
clc;
close all;
load('C:\Users\ndy102\Desktop\thesis\final_markov-2015-07-26\final_markov\classifiedimage_10_0_9_1836.mat');
N=19 ; C=0 ; rbf = 10; imagenum=1836;% [ RGB,x,en ] = combined_markov_svm(images_save );
addpath('C:\Users\ndy102\Desktop\thesis');
...
for i=1:max_num_iterations
    ...
    [en1,x]=ICM_mixed(c1,c2,value,m,n,x,clm);
    en(i)=en1;
...
x1=double(x);
[r1,c1] = find(x==1);
indices1 = sub2ind(size(x), r1, c1);
x1(indices1)=10;
[r2,c3] = find(x==2);
indices2 = sub2ind(size(x), r2, c3);
x1(indices2)=20;
Ig11=double(Ig1);
diff=Ig11-x1;
...
formatSpec = ' %d & %d &%2.2f &%2.2f &%2.4f &%2.1f &%2.1f &%2.1f &%2.1f &%2.2f &%d ';

fprintf(formatSpec, i,N, C, rbf , c2 ,TP ,TN,FP,FN,Accuracy,imagenum)
% sprintf('%2.1f & %2.2f &%2.2f &%2.2f &%2.1f &%2.1f &%2.1f &%2.1f &%2.1f &%2.1f', i,N, C, rbf , c2 ,TP ,TN,FP,FN,Accuracy);
disp('\\ \hline');
sprintf('\n');
filesave=['data_param_',num2str(rbf),'_',num2str(C),'_',num2str(c2*10),'_',num2str(imagenum)];
