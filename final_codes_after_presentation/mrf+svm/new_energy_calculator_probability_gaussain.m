function [ E1 ] = new_energy_calculator_probability_gaussain(mu,sigma,value,m,n,y,x,c1,c2)
...
E1(:,classnum) = -c1*((-0.5*(diff_i*(inv(sigma_i+10^-5)).*diff_i))+(-0.5*log(det(sigma_i)))+(-0.5*log(2*pi)));
